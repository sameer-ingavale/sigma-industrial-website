  ...
  return (
    <>
      <div className="mx-auto max-w-6xl px-5 sm:px-8 pt-6 text-xs text-gray-500">
        <Link href="/brands" className="hover:text-navy">All Brands</Link>
      </div>
          ...
          <div className="flex items-center gap-6">
            {brand.logo && (
              <div className="relative w-32 aspect-[3/2] border border-gray-200 rounded-md flex-shrink-0">
                <Image src={assetUrl(brand.logo)} alt={brand.name} fill className="object-contain p-2" />
              </div>
            )}
      ...
      <section className="mx-auto max-w-6xl px-5 sm:px-8 py-10">
        {brandProducts.length === 0 ? (
          <p className="text-gray-700">No listings here yet — send us your spec directly.</p>
        ) : (
...
function Credential({ label, url }) {
  if (url) {
    return (
      ...
      >
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" className="text-trust" aria-hidden="true">
