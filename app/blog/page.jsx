      ...
      <section className="mx-auto max-w-3xl px-5 sm:px-8 pt-8">
        <div className="flex flex-wrap gap-2">
          ...
          {blogCategories.map((cat) => (
            <FilterPill key={cat.slug} href={`/blog?category=${cat.slug}`} active={activeCategory === cat.slug} label={cat.label} />
          ))}
        </div>
      </section>
          ...
          <div className="divide-y divide-gray-200">
            {posts.map((post) => {
              ...
              return (
                ...
                >
                  <div className="relative w-full aspect-[16/9] bg-gray-50 rounded-md overflow-hidden">
                    {post.image ? (
                      <Image src={assetUrl(post.image)} alt={post.title} fill sizes="(max-width: 768px) 100vw, 700px" className="object-cover" />
                    ) : (
                      <span className="absolute inset-0 flex items-center justify-center text-xs text-gray-400">No photo</span>
                    )}
                  </div>
                  <div className="mt-4">
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      {category && <span className="text-navy font-medium">{category.label}</span>}
                      <span>
                    ...
                    <p className="text-gray-700 mt-2 leading-relaxed">{post.excerpt}</p>
                    <span className="text-sm text-accent font-medium mt-3 inline-block">Read more →</span>
                  </div>
                </Link>
              );
            })}
  ...
  return (
    <Link
      ...
      className={`text-sm px-3 py-1.5 rounded-md border ${
        active ? 'bg-navy text-white border-navy' : 'border-gray-300 text-gray-700 hover:border-navy'
      }`}
    >
      {label}
    </Link>
